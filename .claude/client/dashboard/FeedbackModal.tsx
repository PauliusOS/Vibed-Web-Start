...
import { MessageSquare, Send, Check, Clock, X } from "lucide-react";
...
export function FeedbackModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setTimeout(() => onOpenChange(false), 2000); // Auto-close after 2 seconds
    } catch (error) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[600px] bg-[rgb(10,10,14)] border-white/[0.1] text-white">
        <DialogHeader>
        ...
        <div className="mt-4">
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-2">
              <label htmlFor="feedback-modal" className="text-sm font-medium text-white/70">
                Your Message
              </label>
              <textarea
                id="feedback-modal"
