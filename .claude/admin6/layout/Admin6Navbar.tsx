...
const navItems = [
  { label: "Overview", href: "/admin6/uno", icon: LayoutDashboard },
  { label: "Campaigns", href: "/admin6/uno/list", icon: FolderKanban },
  { label: "All Campaigns", href: "/admin6/uno/campaigns", icon: Target },
  { label: "Analytics", href: "/admin6/uno/analytics", icon: BarChart3 },
  { label: "Invites", href: "/admin6/uno/invitations", icon: Mail },
];

export function Admin6Navbar() {
  ...
  const isActive = (href: string) => {
    if (href === "/admin6/uno") return pathname === "/admin6/uno";
    return pathname.startsWith(href);
  };

  // Get current page title for breadcrumb
  const getCurrentPage = () => {
    if (pathname === "/admin6/uno") return null;
    const item = navItems.find(item => pathname.startsWith(item.href) && item.href !== "/admin6/uno");
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-black/70 backdrop-blur-[10px]">
      ...
      <div className="border-b border-white/[0.08]">
        <div className="flex h-16 items-center px-5 sm:px-6 gap-4">
          {/* Logo + Breadcrumb */}
          <div className="flex items-center gap-3">
            <Link href="/admin6/uno" className="flex items-center gap-2 group">
          ...
          <div className="flex items-center gap-2.5">
            ...
            {/* Settings */}
            <Link
              href="/admin6/uno/settings"
              className={cn(
                "p-2 rounded-lg transition-all",
                pathname.startsWith("/admin6/uno/settings")
      ...
      {/* Tab navigation */}
      <div className="border-b border-white/[0.08]">
        <div className="flex items-center gap-0.5 px-5 sm:px-6">
          {navItems.map((item) => {
...
export function Admin6MobileNav() {
  const pathname = usePathname();

  const mobileItems = [
    { label: "Home", href: "/admin6/uno", icon: LayoutDashboard },
    { label: "Campaigns", href: "/admin6/uno/list", icon: FolderKanban },
    { label: "New", href: "/admin6/uno/new", icon: Plus, special: true },
    { label: "Analytics", href: "/admin6/uno/analytics", icon: BarChart3 },
    { label: "Invites", href: "/admin6/uno/invitations", icon: Mail },
  ];
