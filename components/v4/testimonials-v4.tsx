...
export function TestimonialsV4() {
  ...
  return (
    <section id="testimonials" className="relative py-20 sm:py-24 md:py-32 border-t border-white/[0.07] overflow-hidden bg-[#0a0d14]">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-12 sm:mb-16">
          ...
          <motion.p
            ...
            className="text-[#d5dbe6]/60 text-base sm:text-lg"
          >
      ...
      <div className="flex animate-scroll-creators">
        {duplicatedCreators.map((creator, index) => (
          <div key={index} className="flex-shrink-0 w-64 mx-4">
            {/* Creator Photo */}
            <div
              className="relative aspect-[3/4] rounded-[24px] overflow-hidden mb-4 border border-white/[0.07]"
              style={{
            ...
            >
              {creator.image ? (
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <span className="text-[#d5dbe6]/20 text-sm">Photo</span>
                </div>
              )}
            </div>
            ...
            <div className="space-y-2">
              <h3 className="text-[#d5dbe6] font-semibold text-base">
                {creator.name}
              </h3>
                ...
                {creator.socials.map((social, socialIndex) => (
                  social.followers && (
                    <div key={socialIndex} className="flex items-center gap-1.5">
                      <span className="text-[#d5dbe6]/50">{getSocialIcon(social.platform)}</span>
                      <span className="text-[#d5dbe6]/50">{social.followers}</span>
                    </div>
                  )
                ))}
              </div>
